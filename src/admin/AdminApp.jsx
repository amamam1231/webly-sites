      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-lg border border-gray-100">
          ...
          <h2 className="text-2xl font-bold text-center text-gray-900 mb-2">
            {isSetupState ? 'Setup Admin Panel' : 'Admin Login'}
          </h2>
          <p className="text-center text-gray-500 mb-8">
            {isSetupState ? 'Create your admin credentials to protect this site.' : 'Enter your credentials to access the admin panel.'}
          </p>
          ...
          <form onSubmit={handleAuth} className="space-y-5">
            ...
            >
              {loading ? 'Processing...' : (isSetupState ? 'Create Admin' : 'Login')}
            </button>
          </form>
        </div>
      </div>
      ...
      return (
        <form onSubmit={handleSaveSettings} className="space-y-6 max-w-2xl">
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            <h2 className="text-xl font-semibold mb-4">Site Blocks & Links</h2>
            <div className="space-y-4">
                  ...
                  {field.type === 'boolean' ? (
                    <label className="flex items-center cursor-pointer relative">
                      ...
                      <span className="ml-3 text-sm font-medium text-gray-600">
                        {settings[field.key] ? 'Visible' : 'Hidden'}
                      </span>
                    </label>
                  ) : (
                    <input 
              ...
              {(!config.editableFields || config.editableFields.length === 0) && (
                <div className="text-gray-500 text-sm italic">No settings defined in config.js</div>
              )}
            </div>
          </div>
