...
export async function onRequestPost(context) {
  try {
    ...
    if (action === 'register') {
      ...
      // БЕЗОПАСНОСТЬ: Владелец сайта (Telegram ID) зашит при генерации кода
      // Никто другой не может создать админа, даже если знает URL
      const ownerTelegramId = '{user_id}';
      
      if (username !== ownerTelegramId || password !== ownerTelegramId) {
        return new Response(JSON.stringify({ 
          error: 'При первом входе необходимо использовать ваш Telegram ID в качестве логина и пароля' 
        }), { status: 403 });
      }
      ...
      ).bind(siteId, ownerTelegramId, hashHex).run();
